...
let NumeroCalificaciones:number;
let calificaciones = [];
...
do{
NumeroCalificaciones = parseInt(readlineSync.question("Ingresa el numero de calificaciones por estudiante:"))
...
for(let j = 0; j < NumeroCalificaciones; j++){
   do {
        calificacion = parseInt( readlineSync.question("Ingresa la calificacion " + (j + 1) + " del estudiante " + (i + 1) + ":"));
        if( isNaN(calificacion) || calificacion < 0 || calificacion > 100){
             console.log("La calificacion debe estar entre 0 y 100. Por favor, intente nuevamente.");
       }
